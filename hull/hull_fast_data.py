...
'''
            # convex hull edge
            tri = self.make_triangulation(root["triangulations"][0])
            convex_hull = dict()
            convex_hull["instance_uid"] = self.instance_name
            ch_pts_x=[]
            ch_pts_y=[]
            ch_pts=set()
            for e in tri.edges:
                p1, p2 = e
                t1 = tri.edge_to_face.get((np.int64(p1) <<32) | np.int64(p2))
                t2 = tri.edge_to_face.get((np.int64(p2) <<32) | np.int64(p1))
                if t1 is None or t2 is None:
                    ch_pts.add(p1)
                    ch_pts.add(p2)
            for p in ch_pts:
                ch_pts_x.append(self.pts_x[p])
                ch_pts_y.append(self.pts_y[p])

            convex_hull["pts_x"] = ch_pts_x
            convex_hull["pts_y"] = ch_pts_y

            folder = 'convex_hull_json/'
            with open(folder + self.instance_name+".convex_hull"+".json", "w", encoding="utf-8") as f:
                json.dump(convex_hull, f, indent='\t')
            exit(0)
...
class FastData:
    ...
    def ReadData(self):
        if "solution" not in self.input:
            ...
            for i in range(self.num_tris):
                restore_tri = self.triangulations[i].fast_copy()
                new_pfp_small=[]

                start=time.time()
                new_pfp_a1 = self.small_pfp(i, -1)
                new_pfp_a2 = self.small_pfp_rev(-1, i)

                for pfp in new_pfp_a1:
                    for flp in pfp:
                        self.triangulations[i].flip(flp[0], flp[1])
                new_pfp_small = new_pfp_a1

                self.triangulations[i] = restore_tri.fast_copy()
                for pfp in new_pfp_a2:
                    for flp in pfp:
                        self.triangulations[i].flip(flp[0], flp[1])
                #new_pfp_small = new_pfp_a2
                exit(0)
                new_pfp_a1 = self.parallel_flip_path2(i, -1)
                new_pfp_a2 = self.parallel_flip_path_rev2(-1, i)

                if len(new_pfp_a1) < len(new_pfp_a2):
                    new_pfp[i] = new_pfp_small + new_pfp_a1
                else:
                    new_pfp[i] = new_pfp_small + new_pfp_a2

                new_pfp[i] = [ [tuple(map(int, t)) for t in sublist] for sublist in new_pfp[i]]
    ...
    def small_pfp(self, start_idx, target_idx):
        tri = self.triangulations[start_idx].fast_copy()
        tri_target = self.triangulations[target_idx].fast_copy()

        mean_x = sum(self.pts_x)/self.num_pts
        div_x = [False if x < mean_x else True for x in self.pts_x]
        pfp1=[]
        pfp2=[]
        last_pfp=[]
        for div in (True, False):
            ...
            while True:
                ...
                div_e = [True if div_x[p1]==div and div_x[p2]==div else False for (p1,p2) in tri.edges]
                edges = [e for e, condition in zip(Edges, div_e) if condition]
                ...
                for e in flips:
                    ...
                    e1 = tri.flip(p1, p2)
                    k1, k2 = e1
                    if div_x[k1]!=div and div_x[k2]!=div:
                        tri.flip(k1, k2)
                    else:
                        remained_e.append(e)
                #pfp.append(flips)
                pfp.append(remained_e)
            if div: pfp1 = pfp
            else: pfp2 = pfp

        if len(pfp1) < len(pfp2):
            last_pfp = pfp2
        else:
            last_pfp = pfp1
        return last_pfp

    def small_pfp_rev(self, start_idx, target_idx):
        tri = self.triangulations[start_idx].fast_copy()
        tri_target = self.triangulations[target_idx].fast_copy()
        mean_x = sum(self.pts_x)/self.num_pts
        div_x = [False if x < mean_x else True for x in self.pts_x]
    ...
    def WriteData(self):
        ...
        inst["meta"] = {"dist": sum([len(self.pFlips) for pFlip in self.pFlips])} # , "input": self.input}
