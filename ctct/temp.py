...
def _parallel_flip_path(f_pts, f_nei, e2f, adj, tg_f_pts, tg_f_nei, tg_e2f, tg_adj, pts_coor, depth, reverse):
    ...
    visited_edge = Dict.empty(typ.int64, typ.bool)
    for E_key in e2f.keys():
        visited_edge[E_key] = False
    ...
    while True:
        ...
        for E_key in e2f.keys():
            ...
            key13 = (q1<<32)|q3
            key31 = (q3<<32)|q1
            if key13 in visited_edge:
                if visited_edge[key13]==True: continue
            if key31 in visited_edge:
                if visited_edge[key31]==True: continue
            visited_edge[key13]=True
            visited_edge[key31]=True
...
@numba.njit
def _oled_parallel_flip_path(f_pts, f_nei, e2f, adj, tg_f_pts, tg_f_nei, tg_e2f, tg_adj, pts_coor, depth, reverse):
    pfp = List()
    pfp.append(List([(1,2)]))
    pfp.pop(0)
    while True:
