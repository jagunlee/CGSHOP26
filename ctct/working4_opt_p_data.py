...
def _flip_score_fast(e, p2, p4, tg_f_pts, tg_f_nei, tg_e2f, tg_adj, depth, pts_coor):
    ...
    if depth==1:
        #if e==(60, 68):
        #    print(e, t2, n_cross, " = ", ori_cross, " - ", new_cross, " in score_fast")
        return m_score
...
class FastData:
    ...
    def findCenterGlobal(self):
        ...
        print()
        start=time.time()
        with ProcessPoolExecutor(initializer=init_worker_fcg, initargs=(self.pts,),max_workers=2) as exe:
            for i in range(num):
                ncand =[]
                nscore = 0
                tri = mtriangulations[i]
                ...
                #### parallel ####
                F_E_array = prepare_edge_array(F_E[i], tri.edge_to_face, tri.face_pts)

                j_list = [j for j in range(num) if j!=i]
                chunks = np.array_split(j_list, 4)

                final_total_scores = np.zeros(len(F_E_array), dtype=np.int64)
                g_futures=[]

                for chunk in chunks:
                    g = exe.submit(fcg2, i, chunk.tolist(), F_E_array, mtriangulations)
                    g_futures.append(g)
                for g in as_completed(g_futures):
                    final_total_scores += g.result()

                sorted_indices = np.argsort(final_total_scores)[::-1]
                ncand=[]
                for idx in sorted_indices:
                    score = final_total_scores[idx]
                    if score <=0: break
                    edge_info = F_E[i][idx]
                    ncand.append((edge_info, score))

                #ncand.sort(key = lambda x:x[1], reverse=True)
                #print("T", i,": ncand[:5] = ",  ncand[:5])
                print(f"T{i}, {time.time()-start:.2f}s")
                NCAND.append(ncand)
                marked = set()
                flp =[]
        ...
        print(f"first findCenterGlobal takes{time.time()-start:.2f}s")
        ...
        with ProcessPoolExecutor(initializer=init_worker_fcg, initargs=(self.pts,),max_workers=2) as exe:
            while True:
                ...
                for f in as_completed(futures):
                    try:
                        tri_num, nscore, flp, ncand = f.result()
                        #print("count ",count, ": T", tri_num,": ncand = ",  nscore)
                        if  nscore > mscore:
                            mscore = nscore
                            mi = tri_num
                        flips[tri_num] = flp
                ...
                #F_E_array = prepare_edge_array(F_E[current_mi], curr_e2f, mtriangulations[current_mi].face_pts)
                F_E_array = prepare_edge_array(F_E[current_mi], mtriangulations[current_mi].edge_to_face, mtriangulations[current_mi].face_pts)

                j_list = [j for j in range(num) if j!=current_mi]
                chunks = np.array_split(j_list, 4)
                ...
                for chunk in chunks:
                    g = exe.submit(fcg2, current_mi, chunk.tolist(), F_E_array, mtriangulations)
                    g_futures.append(g)
                for g in as_completed(g_futures):
                    final_total_scores += g.result()
                g_futures=[]
...
#def fcg(tri_num, prev_ncand, current_mi, F_E, prev_f_pts, prev_f_nei, prev_e2f, prev_adj,
#        curr_f_pts, curr_f_nei, curr_e2f, curr_adj):
def fcg(tri_num, prev_ncand, current_mi, F_E, prev_mtri, mtriangulations):#, e2f, prev_e2f, curr_e2f):
    #print("fcg: worker, ", tri_num, " started")
    ncand=[]
    ...
    for e in F_E:
        ...
        escore = 0
        prev_e_score, _ = _flip_score_fast(e, p2, p4, prev_f_pts, prev_f_nei, prev_e2f, prev_adj, 1, gb_pts_coor)
        current_e_score, _ = _flip_score_fast(e, p2, p4, curr_f_pts, curr_f_nei, curr_e2f, curr_adj, 1, gb_pts_coor)
    ...
    #            tg_f_pts = gb_mtris[j].face_pts
    #            tg_f_nei = gb_mtris[j].face_nei
    #            tg_e2f=  process_typed_dict(gb_mtris[j].edge_to_face)
    #            tg_adj = gb_mtris[j].adj
    #            score, _ = _flip_score_fast(e, p2, p4, tg_f_pts, tg_f_nei, tg_e2f, tg_adj, 1, gb_pts_coor)
    #            escore +=score
    #        if escore >0:
    #            ncand.append((e, escore))
    ncand.sort(key = lambda x:x[1], reverse=True)
    ...
    for (p1, p2), escore in ncand:
        ...
        marked.add(t2)
        nscore += escore
    return tri_num, nscore, flp, ncand
...
def fcg2(tri_num, j_list, prepared_E, mtriangulations):
    #print("fcg2: worker, ", j_list, " started")
    ...
    for j in j_list:
        ...
        tg_e2f= mtris_e2f[j]
        scores = _T_flip_score_fast(1, gb_pts_coor, tg_f_pts, tg_f_nei, tg_adj, tg_e2f, prepared_E)
        total_local_scores += scores

    return total_local_scores

@numba.njit
def _T_flip_score_fast(depth, pts_coor, f_pts, f_nei, adj, e2f, prepared_E):
