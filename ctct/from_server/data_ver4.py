...
class FastData:
    ...
    #def my_total_flip_score(self, tri:FastTriangulation, mtriangulations,tri_target_list, e_job_list, chunk_num):
    def my_total_flip_score(self,  current_mi, mtriangulations,tri_target_list, e_job_list, chunk_num):
        ...
        return chunk_num, e_job_total_score
        ...
        if parallel2==False:
            ...
            with ThreadPoolExecutor(initializer=init_worker_fcg, initargs=(self.pts,),max_workers=cpus) as exe:
                while True:
                    ...
                    e_idx_list = []
                    start2_0=time.time()
                    #for chunk in chunks(F_E[current_mi], 1000):
                    #    e_idx_list.append(chunk)
                    for fe_idx in range(0, len(F_E[current_mi]), 1000):
                        e_idx_list.append(F_E[current_mi][fe_idx:fe_idx + 1000])
                    print(f"\tchunk {time.time()-start2_0:.2f}s")
                    tart2=time.time()
                    for chunk_num, edge_job_list in enumerate(e_idx_list):
                        #f = exe.submit(self.my_total_flip_score, tri, mtriangulations, target_list, edge_job_list, chunk_num) #tri is mtriangulations[mi]
                    ...
                    for f in as_completed(futures):
                        try:
                            chunk_num, e_job_total_score = f.result()
                            #print(f"C{chunk_num}: {time.time()-start2_0:.2f}s", end=' ', flush=True)
                    ...
                    F_E[mi] = fe
                    fe=[]
                    #print(f"rest {time.time()-start3:.2f}s")
