...
class FastData:
    ...
    def random_compute_fpd_replace(self, cpus):
        ...
        with ProcessPoolExecutor(
            ...
            for f in as_completed(futures):
                try:
                    #tri_num, updated_seq, ttt, frequ = f.result()
                    #print(f"T{tri_num} 2ps takes {ttt:.2f}s: {frequ}")
                    tri_num, updated_seq, ttt= f.result()
                    #print(f"T{tri_num}: {ttt:.2f}s", end=' ', flush=True)
    ...
    def findCenterGlobal(self, cpus, parallel1=False, parallel2=False):
        ...
        if parallel1==False:
            ...
            ##with ThreadPoolExecutor(initializer=init_worker_fcg3, initargs=(self.pts,mtriangulations,),max_workers=cpus) as exe:
            #        #### parallel ####
            ...
            with Pool(processes=cpus, initializer=init_worker_fcg3, initargs=(self.pts, mtriangulations,)) as pool:
                results = pool.starmap(fcg3, [(tn, prepare_edge_array(F_E[tn], mtriangulations[tn].edge_to_face, mtriangulations[tn].face_pts)) for tn in TN])
            for i in range(num):
                tri_num = results[i][0]
                NCAND[tri_num] = results[i][1]
        ...
        #with ThreadPoolExecutor(initializer=init_worker_fcg4, initargs=(self.pts,mtriangulations,NCAND,),max_workers=cpus) as exe:
        #    g_futures=[]
        #    for tn in TN:
        ...
        with Pool(processes=cpus, initializer=init_worker_fcg4, initargs=(self.pts, mtriangulations,NCAND,)) as pool:
            results = pool.map(fcg4, TN)
        for i in range(num):
            tri_num = results[i][0]
            nscores[tri_num] = results[i][1]
            flips[tri_num] = results[i][2]
        ...
        if parallel2==False:
            ...
            with Pool(processes=cpus, initializer=init_worker_fcg, initargs=(self.pts,)) as pool:
                while True:
                    #count_start=time.time()
                    mscore=0
                    current_mi = mi
                    ...
                    results = pool.starmap(fcg, [(fi, NCAND[i], F_E[fi], mtriangulations[fi], prev_mtri, mtriangulations[mi]) for fi in range(num) if fi!=current_mi])
                    nscores=[0 for i in range(num)]
                    for i in range(num):
                        tri_num = results[i][0]
                        nscores[tri_num] = results[i][1]
                        flips[tri_num] = results[i][2]
                        NCAND[tri_num] = results[i][3]
                    mscore = max(nscores)
                    mi = nscores.index(mscore)
                    ...
                    #        #print(f"T{tri_num}", end=' ', flush=True)
                    #        if  nscore > mscore:
                    #            mscore = nscore
                    #            mi = tri_num
                    #        flips[tri_num] = flp
                    ...
                    #    f = exe.submit(self.total_flip_score, tri, target_list, e)
                    #    futures.append(f)
                    #for f in as_completed(futures):
                    #    try:
                    #        escore = f.result()
    ...
    def WriteData(self):
        ...
        #verify
        #org_input = '/home/gm1225/PFD/data/benchmark_instances/'+self.instance_uid+'.json'
        org_input = '/Users/hyeyun/PFD/hyeyun_git/data/benchmark_instances/'+self.instance_uid+'.json'
        with open(org_input, "r", encoding="utf-8") as f:
            root=json.load(f)
