...
class FastData:
    ...
    def my_total_flip_score(self, tri:FastTriangulation, tri_target_list, e_job_list, chunk_num):
            ...
            with ThreadPoolExecutor(initializer=init_worker_fcg, initargs=(self.pts,),max_workers=cpus) as exe:
                while True:
                    ...
                    for chunk_num, edge_job_list in enumerate(e_idx_list):
                        f = exe.submit(self.my_total_flip_score, tri, target_list, edge_job_list, chunk_num) #tri is mtriangulations[mi]
                        futures.append(f)
                    for f in as_completed(futures):
                        try:
                            #e, escore = f.result()
                    ...
                    print(f"{count}: {time.time()-count_start:.2f}s")
                    count+=1
            #print(end='\n')
