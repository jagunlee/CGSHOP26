...
class FastData:
        ...
        while True:
            ...
            for i in range(num):
                ...
                for e in F_E[i]:
                    ...
                    for j in range(num):
                        ...
                        if e==(4, 6) and count>=1:
                            print("# e = ", e, "T", j,"mi = ", mi,": prev_escore = ??, current_escore = ", score)
                        escore +=score
                    if escore >0:
                        ncand.append((e, escore))
                ncand.sort(key = lambda x:x[1], reverse=True)
                if count>0 and count<=2:
                    print("T", i,", mi = ", mi, ", ncand = ",  ncand)
                elif count==0:
                ...
                if nscore > mscore:
                    mscore = nscore
                    mi = i # mi가 업데이트 되지만, for loop 끝나기 전까진 아무 영향 없음!
            ...
            for ee in tri.edges:
                if self.flippable(tri, ee):
                    fe.append(ee)
            F_E[mi] = fe
            print("count, mscore, mi = ", count, mscore, mi)
            print()
            count+=1
        self.pFlips=[]
        for i in range(num):
    ...
    def random_new_center(self):
        ...
        while param < max_dist *2:
            ...
            print(f"findCenterGlobal takes {time.time()-start:.2f}s")
            for i in range(self.num_tris):
