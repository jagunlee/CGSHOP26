...
def process_(tri_num):
    ...
    for seq_iter in range(1, len(seq)):
        ...
        for e in seq[seq_iter]:
            local_T.flip(e[0], e[1])
    updated_pFlips = random.choice(cands)
...
class FastData:
    ...
    def findCenterGlobal(self, parallel1=False, parallel2=False):
        ...
        if parallel1==False:
            ...
            with ProcessPoolExecutor(initializer=init_worker_fcg, initargs=(self.pts,),max_workers=3) as exe:
            ...
            while True:
                ...
                for i in range(num):
                    ...
                    if nscore > mscore:
                        ...
                        mi = i
                if mscore ==0: break
                #print(f"cnt{count}: {time.time()-count_start:.2f}s", end=' ', flush=True)
                #count+=1
                pfps[mi].append(flips[mi])
                prev_mtri = mtriangulations[mi].fast_copy()
                for e in flips[mi]:
