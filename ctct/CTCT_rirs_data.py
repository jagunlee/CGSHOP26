    ...
    def ReadData(self):
        if "solution" not in self.input:
            ...
            for i in range(len(self.triangulations)):
                print("T", i, "----------")
                tmp_tri = self.triangulations[i].fast_copy()
                ctct_dist = 0
                old_dist = len(self.pFlips[i])

                shorter_pfp=self.pFlips[i]
                for j in range(len(self.pFlips[i])):
                    print(j, "th pfp:", end=' ', flush=True)
                    for flp in self.pFlips[i][j]:
                        tmp_tri.flip(flp)
                    ctct_pfp1 = self.parallel_flip_path_rev2(self.center, tmp_tri)
                    ctct_pfp2 = self.parallel_flip_path_rev2(tmp_tri, self.triangulations[i])
                    cur_dist = len(ctct_pfp1)+len(ctct_pfp2)
                    if cur_dist < old_dist:
                        print("shorter! = ", old_dist, " -> ", cur_dist, end='\n')
                        shorter_pfp= ctct_pfp2 + ctct_pfp1
                        old_dist = cur_dist
                    else:
                        print(old_dist, cur_dist, end='\n')
                new_pfp[i] = shorter_pfp
