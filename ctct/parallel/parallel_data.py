...
@numba.njit
def _flip(p1, p2, f_pts, f_nei, e2f, adj): # flip (p1,p2) -> new edge (p3, p4)
    key12 = (p1 << 32) | (p2)
    key21 = (p2 << 32) | (p1)
    t1 = e2f[key12]
    t2 = e2f[key21]
...
class FastData:
    ...
    def error_findCenterGlobal(self):
        mtriangulations = [t.fast_copy() for t in self.triangulations]
        num = len(self.triangulations)
        pfps = [ [] for _ in range(num)]
    ...
    def WriteData(self):
        ...
        current_dist = sum([len(pFlip) for pFlip in self.pFlips])
        inst["meta"] = {"dist": current_dist}
        ...
        if errors != []:
            print(errors)
            exit(0)
        #else: print("No errors in WriteData()")
        opt_folder = "hy_opt"
