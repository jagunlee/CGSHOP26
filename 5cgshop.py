...
def local_search_on_object(db, dt, idx, centerT, T0_flip, first):
    #print(first, "th local_search_on_object, PID: ", os.getpid())
    objects=[]
    rewards=[]
    rew = 0 # 839 for rirs-1500-50-49040875
    #pool_flips= deepcopy(flips)
    pool_centerT = deepcopy(centerT)
    if first>0:
        ## replace T with shortest pfd to current centerT
        #tmp = deepcopy(dt.triangulations[idx])
        #dt.triangulations[idx] = pool_centerT
        ## find a new center
        #newCT = dt.findCenter()
        ## restore
        ...
        flip_len=[len(f) for f in new_flip]
        ...
        obj = convert_to_string(new_flip[0]) #hy: T_0 to centerT

    #rew = 23-dt.computeDistanceSum(pool_centerT)
    if first==0:
        obj = convert_to_string(T0_flip)
    new = reward(db, obj)
    if new:
       objects.append(obj)
       rewards.append(rew)
...
def local_search_from_decoded(db, inst_file, path, input_file):
    ...
    for obj in lines:
        ...
        obj, rew = local_search_on_object(db, dt, min_len_idx, centerT, flips[0], 1)
...
def local_search(db, path, input_file):
    ...
    single_thread_rew =[]
    with Pool() as pool:
        obj_rew = pool.starmap(local_search_on_object, [(db, dt, min_len_idx, centerT, flips[0], i) for i in range(10)])
    for obj, rew in obj_rew:
        single_thread_obj.append(obj)
        single_thread_rew.append(rew)
    #for i in range(10):
    #    print(i, " in local_search")
...
def get_parser():
    ...
    parser.add_argument('--max-output-length', type=int, default=160, help="maximum output length") #hy: random_instance_444
