...
function local_search!(db, lines, start_ind, nb=nb_local_searches)
    ...
    @threads for obj in pool
        println("hy: threadid() = ", threadid())
        list_obj, list_rew = local_search_on_object(db, obj)
        append!(local_search_results_threads[threadid()-1][1], list_obj) #hy: with thread,-1. otherwise, remove -1.
        append!(local_search_results_threads[threadid()-1][2], list_rew)
    end
    #@sync @distributed for obj in pool
    #results = @distributed (vcat) for obj in pool
    #    list_obj, list_rew = local_search_on_object(db, obj)
    #    [(list_obj, list_rew)]
    #end
    # we update the dictionaries
    # hy
    #for (list_obj, list_rew) in results
    #    add_db!(db, list_obj, list_rew)
    #end
    for j=1:nthreads()
...
println("hy: nb_local_searches = ", nb_local_searches)
num_initial_empty_objects = parse(Int,ARGS[3])
println("hy: num_initial_empty_objects = ", num_initial_empty_objects)
final_database_size = parse(Int,ARGS[4])
println("hy: final_database_size = ", final_database_size)
target_db_size = parse(Int,ARGS[5])
println("hy: target_db_size  = ", target_db_size)
main()
