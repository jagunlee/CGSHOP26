...
def get_parser():
    ...
    parser.add_argument('--num_initial_empty_objects', type=int, default=500, help='number of initial rollouts, before the first learning loop')
    parser.add_argument('--final_database_size', type=int, default=50, help='training set size')
    ...
    parser.add_argument('--nb_threads', type=int, default=2, help='Number of cpu threads')
    parser.add_argument('--nb_local_searches', type=int, default=10, help='This only matters when using multithreading, then it should be a multiple of the number of threads used')
...
if __name__ == '__main__':
    ...
    if initial_gen == 0:
        ...
        subprocess.run(["julia","search_pfd.jl", args.dump_path, str(args.nb_local_searches), str(args.num_initial_empty_objects), str(args.final_database_size), str(args.target_db_size)])#hy: create search_output_1.txt from search_pfd.jl
        print("hy: ------ now tokenize ")
    ...
    if os.path.isfile(model_path): # Note: if we sample-only then we also assume we are resuming#hy???
