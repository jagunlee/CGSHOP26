...
class Data:
    ...
    def ReadData(self):
        ...
        # print("--------------------ReadData--------------------")
            ...
            for i in range(len(self.triangulations)):
                ...
                #hy2
                new_pfp1 = self.hy_parallel_flip_path(self.triangulations[i], self.center)
                exit(0)
                new_pfp2 = self.hy_parallel_flip_path(self.center, self.triangulations[i])
    ...
    #hy
    def hy_parallel_flip_path(self, tri1:Triangulation, tri2:Triangulation):
        tri = copy.deepcopy(tri1)
        pfp = []
        prev_flipped=set()
        while True:
            t_edges = dict.fromkeys(tri.edges,0)
            highest_score = len(tri.edges)
            NF_e=[]
            for e in t_edges.keys():
                #if e in prev_flipped:
                #    t_edges[e] =-1 #do not flip
                if e in tri2.edges:
                    t_edges[e] = -1 #do not flip
                elif self.flippable(tri, e):
                    t_edges[e] +=1
                else:
                    t_edges[e] -=1 #do not flip
                    NF_e.append(e)

            highest_score = max(len(NF_e), 1)
            for e in t_edges.keys():
                if t_edges[e]==1:
                    e1 = tri.flip(e)
                    if e1 in tri2.edges:
                        t_edges[e] +=len(tri.edges)
                        tri.flip(e1)
                        continue
                    for ee in t_edges.keys():
                        if ee==e: continue
                        if t_edges[ee]>=1 and self.flippable(tri, ee)==False:
                            t_edges[e]-=1
                        elif ee in NF_e and self.flippable(tri, ee)==True:
                            t_edges[e]+=1
                    tri.flip(e1)

            t_edges = dict(sorted(t_edges.items(), key=lambda item: item[1], reverse=True))
            print("1", t_edges)

            flips = []
            marked = set()
            for e in t_edges.keys():
                if t_edges[e]<0:continue
                (p1, p2) = e
            ...
            print("2", t_edges)
            pfp.append(flips)
            print("intersection = ", len(tri.edges - tri2.edges) , "/", len(tri.edges))
            print()
            if tri.edges == tri2.edges: break
