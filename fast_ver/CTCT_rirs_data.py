    ...
    def ReadData(self):
        if "solution" not in self.input:
            ...
            for i in range(len(self.triangulations)):
                print("T", i, "----------")
                new_pfp[i]=self.parallel_flip_path_rev2(self.center, self.triangulations[i])
                tmp_tri = self.triangulations[i].fast_copy()
                ctct_dist = 0
                old_dist = len(new_pfp[i])

                shorter_pfp=new_pfp[i]
                for j in range(len(new_pfp[i])):
                    print(j, "th pfp:", end=' ', flush=True)
                    for flp in new_pfp[i][j]:
                        tmp_tri.flip(flp)
